-- "addons\\homigrad-weapons\\lua\\weapons\\weapon_870_a.lua"
...
function SWEP:Reload()
    ...
    if !self.Pumped and self.PumpTarg == 0 then
        ...
        if SERVER then
            sound.Play("zcitysnd/sound/weapons/toz_shotgun/handling/toz_pumpback.wav",self:GetPos(),70,100,1,0)
        end
        hg.PlayAnim(self,"pump")
        timer.Simple(0.125,function()
            if SERVER then
                sound.Play("zcitysnd/sound/weapons/toz_shotgun/handling/toz_pumpforward.wav",self:GetPos(),70,100,1,0)
            end
            self.PumpTarg = 0
            self.Pumped = true
