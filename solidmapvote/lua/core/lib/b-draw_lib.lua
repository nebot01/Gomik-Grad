-- "addons\\solidmapvote\\lua\\core\\lib\\b-draw_lib.lua"
...
local function fetch_asset(url)
	...
	if not string.StartWith(lower, "http://") and not string.StartWith(lower, "https://") then
		local mat = Material(url)
		mats[url] = (mat and not mat:IsError()) and mat or _error
		return mats[url]
	end
